resource "null_resource" "docker-swarm-manager" {
  ...
  provisioner "file" {
    source      = "/home/alps/socks_deploy/docker-compose-v3.yml"
    destination = "/home/alps/docker-compose.yml"
  }
...
resource "null_resource" "docker-swarm-manager-join" {
  ...
  provisioner "local-exec" { 
    #перед запуском проверим вывод команды в консоль
	command = <<EOT
	echo "Executing command:"
	echo "TOKEN=\$(ssh -i ${var.ssh_credentials.private_key} -o StrictHostKeyChecking=no ${var.ssh_credentials.user}@${yandex_compute_instance.vm-manager[count.index].network_interface.0.nat_ip_address} docker swarm join-token -q worker); echo \"#!/usr/bin/env bash\nsudo docker swarm join --token \$TOKEN ${yandex_compute_instance.vm-manager[count.index].network_interface.0.nat_ip_address}:2377\nexit 0\" >| join.sh"
	TOKEN=$(ssh -i ${var.ssh_credentials.private_key} -o StrictHostKeyChecking=no ${var.ssh_credentials.user}@${yandex_compute_instance.vm-manager[count.index].network_interface.0.nat_ip_address} docker swarm join-token -q worker)
...
resource "null_resource" "docker-swarm-worker" {
  ...
  provisioner "file" {
    source      = "/home/alps/socks_deploy/join.sh"
    destination = "/home/alps/join.sh"
  }
